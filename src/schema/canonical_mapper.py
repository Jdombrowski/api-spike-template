...
class EdgarConceptMapper:
    ...
    def extract_values(self, raw: dict, preferred_unit: str = "USD") -> list[dict]:
        ...
        if not units:
            log.warning("[concept_mapper] no 'units' key in response — empty or unsupported concept")
            return []
        ...
        if preferred_unit not in units:
            log.warning(
                ...
                preferred_unit, available_units
            )
...
def reconcile_entity(edgar_canonical: dict, polygon_canonical: dict) -> dict:
    ...
    name_similar = (
        ...
        if edgar_name and polygon_name else False
    )
