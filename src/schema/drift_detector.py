...
class SchemaDriftDetector:
    ...
    def check(self, record: dict, flat: bool = False) -> list[DriftIssue]:
        ...
        if not self.baseline:
            raise RuntimeError(
                "No baseline set. Call set_baseline_from_profiler() or load() first."
            )
        ...
        current_types = {
            k: type(v).__name__ if v is not None else "NoneType" for k, v in current.items()
        }
    ...
    def check_batch(self, records: list[dict]) -> dict[int, list[DriftIssue]]:
        ...
        return {i: issues for i, r in enumerate(records) if (issues := self.check(r))}
